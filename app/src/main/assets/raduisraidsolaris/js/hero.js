	...
	if( this.life > 0 ) {
		...
		if( this.weapon.fireRateTick < this.weapon.fireRate ){
			...
			if( $.autofire || ( !$.autofire && $.mouse.down ) ){
				$.audio.play( 'shoot' );
				if( $.powerupTimers[ 2 ] > 0 || $.powerupTimers[ 3 ] > 0 || $.powerupTimers[ 4 ] > 0 || $.powerupTimers[ 5 ] > 0 || $.powerupTimers[ 6 ] > 0) {
					$.audio.play( 'shootAlt' );
				}
				...
				for( var i = 0; i < this.weapon.count; i++ ) {
					...
					if( $.powerupTimers[ 2 ] > 0 || $.powerupTimers[ 3 ] > 0 || $.powerupTimers[ 4 ] > 0 || $.powerupTimers[ 5 ] > 0 || $.powerupTimers[ 6 ] > 0) {
						...
						if( $.powerupTimers[ 5 ] > 0 ) { colors.push( 'hsl(' + $.definitions.powerups[ 5 ].hue + ', ' + $.definitions.powerups[ 5 ].saturation + '%, ' + $.definitions.powerups[ 5 ].lightness + '%)' ); }
						if( $.powerupTimers[ 6 ] > 0 ) { colors.push( 'hsl(' + $.definitions.powerups[ 6 ].hue + ', ' + $.definitions.powerups[ 6 ].saturation + '%, ' + $.definitions.powerups[ 6 ].lightness + '%)' ); }
		...
		while( ei-- ) {
			...
			if( enemy.inView && $.util.distance( this.x, this.y, enemy.x, enemy.y ) <= 2*this.radius + enemy.radius ) {
...
/*
		...
		$.ctxmg.restore();
*/
		$.util.fillCircle( $.ctxmg, this.x, this.y, this.radius/2, fillStyle );
		
		
		$.ctxmg.save();
		$.ctxmg.translate( this.x, this.y );
		$.ctxmg.rotate( this.direction  + $.pi / 2);
		var multiplier = 5*this.radius/$.space_ship.width; 
		$.ctxmg.globalAlpha = 0.9;
		$.ctxmg.drawImage($.space_ship, 0- multiplier*$.space_ship.width/2, 0- multiplier*$.space_ship.width/2, 5*this.radius, 5*this.radius);
		...
		$.ctxmg.translate( this.x, this.y );
		$.ctxmg.rotate( this.direction  + $.pi / 2);
		var multiplier = this.radius/(2*$.yutani_image.width); 
		$.ctxmg.globalAlpha = 1.0;
		$.ctxmg.drawImage($.yutani_image, 0- multiplier*$.yutani_image.width/2, 0- multiplier*$.yutani_image.width/8, this.radius/2, this.radius/2);
        $.ctxmg.globalAlpha = 1.0;
		$.ctxmg.restore();
